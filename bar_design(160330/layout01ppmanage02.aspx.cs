...
public partial class layout01ppmanage02 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            conn.Open();
            SqlCommand radiocmd = new SqlCommand("select canpost from CareerForm where CareerFormID=@CareerFormID", conn);
            radiocmd.Parameters.AddWithValue("@CareerFormID", FromIDTemp.Text);
            SqlDataReader sqr = radiocmd.ExecuteReader();
            while (sqr.Read())
            {
                if (sqr["canpost"].ToString() == "1")
                {
    ...
    {
        Response.Redirect("layout01ppmanage01.aspx");
    }
