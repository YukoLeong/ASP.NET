        ...
        Try
            ...
            If a = 1 Then
                ...
                If tmp_ds.Tables("SystemAdmin").Rows.Count <> 0 Then
                    ...
                    Session.Add("sysadminID", tmp_dr("uid"))
                    Response.Redirect("~/SystemAdmin/home.aspx")
                End If



                Dim cmd1 As SqlCommand = New SqlCommand("Select * from Staff_Info where uid=@id and uid=@password order by cat_type desc", conn)
                    ...
                    If Trim(tmp_dr("cat_type")) = "extr" And (Trim(tmp_dr("COD")) = "ESA" Or Trim(tmp_dr("COD")) = "ESCE" Or Trim(tmp_dr("COD")) = "ESAP" Or Trim(tmp_dr("COD")) = "ESLT" Or Trim(tmp_dr("COD")) = "ESEFD" Or Trim(tmp_dr("COD")) = "ESS") Then
                        ...
                        If tmp_ds4.Tables("RoleAccount").Rows.Count <> 0 Then
                            ...
                            If tmp_dr4("position") = "tutor" Then
                                ...
                                Session.Add("position_role", "tutor")
                                Response.Redirect("~/Academic/home.aspx")
                            End If
                        End If
                  ...
                  If Trim(tmp_dr("cat_type")) = "basi" And (Trim(tmp_dr("COD")) = "ESA" Or Trim(tmp_dr("COD")) = "ESCE" Or Trim(tmp_dr("COD")) = "ESAP" Or Trim(tmp_dr("COD")) = "ESLT" Or Trim(tmp_dr("COD")) = "ESEFD" Or Trim(tmp_dr("COD")) = "ESS") Then
                        ...
                        If tmp_ds3.Tables("RoleAccount").Rows.Count <> 0 Then
                            ...
                            If tmp_dr3("position") = "operator" Then
                                ...
                                Session.Add("COD", tmp_dr3("COD"))
                                Response.Redirect("~/Admin/home.aspx")
                            End If

                            If tmp_dr3("position") = "manager" Then
                                Session.Add("Role_Type", "manager")
                                Session.Add("ID", Request("Name"))
                                Session.Add("COD", tmp_dr3("COD"))
                                Response.Redirect("~/Admin/home.aspx")
                            End If

                            If tmp_dr3("position") = "schooladmin" Then
                                Session.Add("Role_Type", "schooladmin")
                                Session.Add("ID", Request("Name"))
                                Session.Add("COD", tmp_dr3("COD"))
                                Response.Redirect("~/Admin/home.aspx")
                            End If
                ...
                Dim tmp_stuid As SqlDataAdapter = New SqlDataAdapter()
                Dim tmp_dsStuid As New DataSet
                Dim tmp_drStuid As DataRow

                Dim cmdStuid As SqlCommand = New SqlCommand("Select TOP 1 * from student_academic_info where netid=@id order by ALUN_NUMERO_SEQ desc ", conn)
                cmdStuid.Parameters.AddWithValue("@id", Request("Name"))
                tmp_stuid.SelectCommand = cmdStuid


                tmp_stuid.Fill(tmp_dsStuid, "student_academic_info")


                If tmp_dsStuid.Tables("student_academic_info").Rows.Count <> 0 Then
                    tmp_drStuid = tmp_dsStuid.Tables("student_academic_info").Rows(0)
                  
                    Session.Add("COD", tmp_drStuid("SCHOOL_CODE"))


                    Dim tmp_daCheck As New SqlClient.SqlDataAdapter("Select * from RoleAccount where uid='" & Request("Name") & "'", conn)
                    Dim tmp_dsCheck As New DataSet
                    Dim tmp_drCheck As DataRow


                    tmp_daCheck.Fill(tmp_dsCheck, "RoleAccount")


                    If tmp_dsCheck.Tables("RoleAccount").Rows.Count <> 0 Then
                        tmp_drCheck = tmp_dsCheck.Tables("RoleAccount").Rows(0)


                        If tmp_drCheck("position") = "operator" Then
                            Session.Add("Role_Type", "operator")
                            Session.Add("ID", Request("Name"))
