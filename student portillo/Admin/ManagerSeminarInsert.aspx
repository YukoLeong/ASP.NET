<%@ Page Language="VB" AutoEventWireup="false" CodeFile="ManagerSeminarInsert.aspx.vb" Inherits="Operator_ManagerSeminarInsert" %>
 ...
 <%--       <asp:LinkButton ID="Home" runat="server" CssClass="blue" 
    ...
    <asp:Label ID="SemYear" runat="server" Text="Seminar Year:" Font-Size="Larger"  Font-Bold="True"></asp:Label>
         ...
         <input type="hidden" id="HiddenText" runat="server" />
            ...
            <td class="style20">
                ...
                <asp:Button ID="Insert" runat="server" CssClass="blue" 
                    style="margin-left: 25px" Text="Register" />
                </td>
              </tr>
     ...
     <asp:SqlDataSource ID="topicSrc" runat="server" 
        ...
        SelectCommand="SELECT distinct [topicName],id FROM [Seminar_Info] where (status=@status and year([startDate])=@year and category=@category)">
        <SelectParameters>
    ...
    <asp:SqlDataSource ID="category" runat="server" 
        ...
        SelectCommand="SELECT  distinct category FROM [Seminar_Info] where (status=@status and year([startDate])=@year and category != '')">
        <SelectParameters>
           <asp:Parameter DefaultValue="Internal" Name="status" />
    ...
    <asp:SqlDataSource ID="year" runat="server" 
        ConnectionString="<%$ ConnectionStrings:EPConnectionString %>" 
        SelectCommand="SELECT  distinct year([startDate]) as year FROM [Seminar_Info]  WHERE (status=@status) ORDER BY year DESC">
       <SelectParameters>
            <asp:Parameter DefaultValue="Internal" Name="status" />
