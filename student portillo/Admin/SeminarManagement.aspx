<%@ Page Language="VB" AutoEventWireup="false" CodeFile="SeminarManagement.aspx.vb" Inherits="Operator_SeminarManagement" %>
...
<body background="../images/church.gif">
  <script type="text/javascript">
    ...
    <asp:SqlDataSource ID="SeminarInfo" runat="server" 
        ...
        SelectCommand="SELECT * FROM [Seminar_Info] WHERE ((year([startDate]) = @startDate) AND ([category] = @category) AND ([status] = @status) AND ([COD] = @COD)) order by startDate desc"
        ...
        <asp:View ID="View3" runat="server">
            ...
            <asp:SqlDataSource ID="SeminarInfoDetail" runat="server" 
                ...
                UpdateCommand="UPDATE [Seminar_Info] SET [topicName] = @topicName, [category] = ISNULL(@category, 'Not define'), [startDate] = @startDate, [duration] = @duration, [organizer] = @organizer, [bonus_point] = @bonus_point,  [update_time] = GETDATE(), [topicNameCht] = @topicNameCht, [categoryCht] = @categoryCht, [organizerCht] = @organizerCht WHERE [id] = @id">
                <DeleteParameters>
                       <asp:Parameter Name="id" Type="Int32" />
                </DeleteParameters>
                <SelectParameters>
