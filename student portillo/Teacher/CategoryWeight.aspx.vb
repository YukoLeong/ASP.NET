Public Class CategoryWeight2
    ...
    Protected Sub GridViewCategory_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles GridViewCategory.SelectedIndexChanged
        ...
        SchoolAttribute.Series(SchoolName).Points.DataBindXY(XValue, YValue1_s)

        ProgrammeAttribute.Series.Item(0).Name = ProgramName
        ProgrammeAttribute.Series(ProgramName).Points.DataBindXY(XValue, YValue1_1)

        ProgrammeAttrPie.Series.Item(0).Name = ProgramName
        ProgrammeAttrPie.Series(ProgramName).Points.DataBindXY(XValue, YValue1_1)


        SchoolAttribute.Series.Item(1).Name = ProgramName
        SchoolAttribute.Series(ProgramName).Points.DataBindXY(XValue, YValue1_1)

        SqlDataSource4.SelectCommand = "SELECT * FROM [CategoryWeight] where school = '" & SchoolName & "' order by programme desc"
